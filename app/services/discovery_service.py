# v1.2.1
# Discovery Engine TURBO - Asinkrona verzija s asyncio semaforima i zaštitom od rušenja.
...
# Funkcija za asinkrono slanje ICMP ping zahtjeva kako bi se provjerio status uređaja na mreži
async def async_ping(ip: str, semaphore: asyncio.Semaphore):
    async with semaphore:
        try:
            ...
            print(f"[!] Ping greška za {ip}: {e}")
            return ip, False

# Sinkrona funkcija koja obrađuje rezultat pinga i upisuje status uređaja te logove u bazu
def process_scan_result(db: Session, ip_str: str, is_online: bool, subnet_id: int):
    try:
        ...
        print(f"[!] Greška pri obradi IP-a {ip_str}: {e}")

# Funkcija koja pokreće cijeli subnet scan asinkrono, ali rezultate obrađuje sinkrono u bazi
